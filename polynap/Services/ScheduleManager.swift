...
class ScheduleManager: ObservableObject {
    ...
    private func loadActiveScheduleFromLocalDatabase() {
        Task {
            do {
                print("ScheduleManager: Repository'den aktif program yükleme başlıyor...")
                if let localSchedule = try await Repository.shared.getActiveSchedule() {
                    print("ScheduleManager: ✅ Repository'den aktif program yüklendi: \(localSchedule.name) (ID: \(localSchedule.id))")
                    DispatchQueue.main.async {
                        self.activeSchedule = localSchedule
                        self.updateNotificationsForActiveSchedule()
                        print("ScheduleManager: activeSchedule atandı ve bildirimler güncellendi")
                    }
                } else {
                    print("ScheduleManager: ❌ Repository'de aktif program bulunamadı!")
                    DispatchQueue.main.async {
                        self.activeSchedule = nil
                    }
                }
            } catch {
                print("ScheduleManager: ❌ Repository'den aktif program yüklenirken hata: \(error)")
                DispatchQueue.main.async {
                    self.activeSchedule = nil
                }
            }
