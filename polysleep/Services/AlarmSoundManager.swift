...
class AlarmSoundManager {
    ...
    private func scanAvailableSounds() {
        guard let resourcePath = Bundle.main.resourcePath else {
            print("PolySleep Debug: Resource path bulunamadı")
            return
        }
        ...
        do {
            ...
            for file in files {
                ...
                if let profile = createSoundProfile(from: fileURL) {
                    ...
                    print("PolySleep Debug: Alarm sesi bulundu: \(profile.name) (\(profile.format), \(profile.duration)s)")
                }
            }
            
        } catch {
            print("PolySleep Debug: Alarm ses dosyaları taranamadı: \(error)")
        }
    ...
    private func createSoundProfile(from url: URL) -> AlarmSoundProfile? {
        ...
        guard allSupportedFormats.contains(fileExtension) else {
            print("PolySleep Debug: Desteklenmeyen format: \(fileName)")
            return nil
        }
        
        // Süre kontrolü
        guard let duration = getAudioDuration(from: url) else {
            print("PolySleep Debug: Ses dosyası süresi okunamadı: \(fileName)")
            return nil
        }
        ...
        if duration > maxDuration {
            print("PolySleep Debug: Uyarı - \(fileName) dosyası \(duration)s uzunluğunda (Apple limiti: \(maxDuration)s)")
        }
    ...
    private func getAudioDuration(from url: URL) -> TimeInterval? {
        do {
            ...
            print("PolySleep Debug: Audio dosya bilgisi okunamadı: \(error)")
            return nil
        }
    }
    ...
    func optimizeSoundFile(inputFileName: String, completion: @escaping (Bool, String?) -> Void) {
        guard let inputURL = Bundle.main.url(forResource: inputFileName.replacingOccurrences(of: ".\(inputFileName.split(separator: ".").last ?? "")", with: ""), withExtension: String(inputFileName.split(separator: ".").last ?? "")) else {
            print("PolySleep Debug: Giriş dosyası bulunamadı: \(inputFileName)")
        ...
        Task {
            do {
                ...
                if status == .haveData, error == nil {
                    ...
                    print("PolySleep Debug: Ses dosyası başarıyla optimize edildi: \(outputFileName)")
                    
                    DispatchQueue.main.async {
                        completion(true, outputFileName)
                    }
                } else {
                    print("PolySleep Debug: Dönüştürme hatası: \(error?.localizedDescription ?? "Bilinmeyen hata")")
                    DispatchQueue.main.async {
                ...
                print("PolySleep Debug: Ses optimizasyonu hatası: \(error)")
                DispatchQueue.main.async {
                    completion(false, error.localizedDescription)
                }
            }
        ...
        Task {
            do {
                ...
                if CMTimeGetSeconds(duration) <= maxDuration {
                    print("PolySleep Debug: Dosya zaten 30 saniye veya daha kısa")
                ...
                if exportSession.status == .completed {
                    print("PolySleep Debug: Ses dosyası 30 saniyeye kırpıldı: \(outputFileName)")
                    DispatchQueue.main.async {
                        completion(true, outputFileName)
                    }
                } else {
                    print("PolySleep Debug: Kırpma hatası: \(exportSession.error?.localizedDescription ?? "Bilinmeyen hata")")
                    DispatchQueue.main.async {
                ...
                print("PolySleep Debug: Ses kırpma hatası: \(error)")
                DispatchQueue.main.async {
                    completion(false, error.localizedDescription)
                }
            }
    ...
    private func createDefaultSounds() {
        ...
        print("PolySleep Debug: Varsayılan alarm sesleri eklendi")
    }
...
extension AlarmSoundManager {
    ...
    func convertSoundWithSimulatedCommand(inputFile: String, completion: @escaping (Bool, String) -> Void) {
        print("PolySleep Debug: Terminal benzeri dönüştürme başlatılıyor...")
