...
class AlarmAudioManager: NSObject, ObservableObject {
    ...
    private func setupAudioSession() {
        do {
            ...
            print("PolySleep Debug: Audio session alarm için optimal ayarlarla yapılandırıldı")
            
        } catch {
            print("PolySleep Debug: Audio session yapılandırma hatası: \(error)")
        }
    }
    ...
    private func validateSoundFile(_ url: URL) -> Bool {
        do {
            ...
            if duration > maxAlarmDuration {
                print("PolySleep Debug: Uyarı - Ses dosyası \(duration)s uzunluğunda, Apple'ın 30s limitini aşıyor")
                return false
            }
            ...
            if !supportedFormats.contains(fileExtension) {
                print("PolySleep Debug: Uyarı - Desteklenmeyen ses formatı: \(fileExtension)")
                return false
            }
            
            print("PolySleep Debug: Ses dosyası geçerli - \(duration)s (\(fileExtension))")
            return true
            
        } catch {
            print("PolySleep Debug: Ses dosyası validasyon hatası: \(error)")
            return false
        }
    }
    ...
    private func findBestSoundFile(soundName: String) -> URL? {
        ...
        for format in priorityFormats {
            if let url = Bundle.main.url(forResource: soundFileName, withExtension: format) {
                if validateSoundFile(url) {
                    print("PolySleep Debug: En uygun ses dosyası bulundu: \(soundFileName).\(format)")
                    return url
                }
            }
        }
        ...
        for format in priorityFormats {
            if let url = Bundle.main.url(forResource: "alarm", withExtension: format) {
                if validateSoundFile(url) {
                    print("PolySleep Debug: Fallback alarm sesi kullanılıyor: alarm.\(format)")
                    return url
                }
            }
        }
        
        print("PolySleep Debug: Uygun ses dosyası bulunamadı")
        return nil
    }
    ...
    func startAlarmAudio(soundName: String = "alarm", volume: Float = 1.0) async {
        guard !isPlaying else {
            print("PolySleep Debug: Alarm sesi zaten çalıyor")
            return
        }
        
        print("PolySleep Debug: Medium yöntemiyle alarm sesi başlatılıyor...")
        ...
        guard let soundURL = findBestSoundFile(soundName: soundName) else {
            print("PolySleep Debug: Geçerli ses dosyası bulunamadı, sistem alarmı kullanılacak")
        ...
        do {
            ...
            if success {
                isPlaying = true
                print("PolySleep Debug: Güçlü alarm sesi başlatıldı - \(soundName) (volume: \(volume))")
                ...
                print("PolySleep Debug: Alarm sesi başlatılamadı")
                stopBackgroundTask()
                await useSystemAlarmSound()
            }
            
        } catch {
            print("PolySleep Debug: Audio player oluşturma hatası: \(error)")
            stopBackgroundTask()
            await useSystemAlarmSound()
        }
    }
    ...
    func stopAlarmAudio() async {
        ...
        do {
            ...
            print("PolySleep Debug: Audio session kapatma hatası: \(error)")
        }
        
        // Background task'i sonlandır
        stopBackgroundTask()
        
        print("PolySleep Debug: Alarm sesi durduruldu")
    }
    
    /// Medium makalesine göre sistem alarm sesi kullan (fallback)
    private func useSystemAlarmSound() async {
        print("PolySleep Debug: Sistem alarm sesi çalınıyor (fallback)")
        ...
        stopBackgroundTask()
        print("PolySleep Debug: Sistem alarm sesi döngüsü tamamlandı")
    }
    ...
    func adjustVolume(_ newVolume: Float) {
        ...
        print("PolySleep Debug: Alarm volume ayarlandı: \(clampedVolume)")
    }
    ...
    private func startBackgroundTask() {
        ...
        print("PolySleep Debug: Background task başlatıldı")
    }
    ...
    private func stopBackgroundTask() {
        if backgroundTaskID != .invalid {
            ...
            print("PolySleep Debug: Background task sonlandırıldı")
        }
    }
}
...
extension AlarmAudioManager: AVAudioPlayerDelegate {
    func audioPlayerDidFinishPlaying(_ player: AVAudioPlayer, successfully flag: Bool) {
        if flag {
            print("PolySleep Debug: Alarm sesi bitti")
        } else {
            print("PolySleep Debug: Alarm sesi hata ile sonlandı")
        }
        isPlaying = false
        stopBackgroundTask()
    }
    ...
    func audioPlayerDecodeErrorDidOccur(_ player: AVAudioPlayer, error: Error?) {
        print("PolySleep Debug: Audio decode hatası: \(error?.localizedDescription ?? "Bilinmeyen hata")")
        isPlaying = false
        stopBackgroundTask()
    }
} 
